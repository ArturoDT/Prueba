...
<%
    if (request.getParameter("serie") != null) {
        ...
        if (c.conectar()) {
            ...
            if (control != 0) {
                ResultSet resul = c.consultas("Select alumno.Id_Grupo from alumno join grupos join periodoesc on grupos.Id_Grupo=alumno.Id_Grupo and "
                        + " grupos.Id_Periodo=periodoesc.Id_Periodo where periodoesc.Estado="+1+" and alumno.Control="+nc);
                int id = 0;
                Date f = null;
                String per = "";
                if (resul != null) {
                    ...
                    ResultSet verificarregistro = c.consultas("Select asistencia.Id_alumno from asistencia join alumno join grupos join periodoesc on asistencia.Id_alumno=alumno.Control "
                            + " and grupos.Id_Grupo = alumno.Id_Grupo and periodoesc.Id_Periodo = grupos.Id_Periodo "
                            + " where periodoesc.Estado="+1+" and asistencia.Id_alumno=" + nc + " and asistencia.FechaE='" + nf + "'");
